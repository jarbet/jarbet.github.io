...
format: 
  revealjs: 
    output-file: 2023-12-15_mars.html
scrollable: TRUE
slide-number: c/t
...
**`r colorize('Goal:', 'steelblue')`** build a flexible yet *interpretable* model like:
...
**`r colorize('Question:', 'steelblue')`** what are 3 things you need to determine when fitting a piecewise linear spline?
...
* The hinge fn comes in a *pair* of terms: \n
...
* > ...the regression surface is built up parsimoniously, using nonzero components locally—only where they are needed. This is important, since one should “spend” parameters carefully in high dimensions, as they can run out quickly ["curse of dimensionality"]. The use of other basis functions such as polynomials, would produce a nonzero product everywhere, and would not work as well [@ESL]
:::
...
**`r colorize('Nonparametric?', 'steelblue')`**
...
:::{.column-body-outset} 
![](figures/forward_step.png){fig-align="center"}
::: 
...
* Selected all `r nrow(evimp(fit.gcv))` predictors, using `r ncol(fit.gcv$bx)` total hinge functions
...
## Using 10-fold CV (5 repeats) to select the model size {.scrollable}
